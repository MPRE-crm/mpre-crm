// app/api/twilio/followUpMissedAppointment.ts
...
export async function followUpMissedAppointment(leadId: string): Promise<void> {
  try {
    ...
    const firstName: string = lead.first_name || lead.name || 'there';
    ...
    const messageBody = `Hi ${firstName}, this is Samantha, your Boise real estate assistant. I noticed you missed your appointment today. Would you like to reschedule? Here are two options: 1) ${option1}  2) ${option2}. Reply with your choice!`;
    ...
    // Optional AI call follow-up (replace with your real call handler URL)
    ...
    console.log(`Follow-up sent to ${firstName} via SMS${callUrl ? ' and AI call' : ''}.`);
  } catch (error) {
