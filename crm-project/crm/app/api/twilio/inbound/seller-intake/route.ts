      ...
      .then((branding) => {
        ...
        oaSocket.send(JSON.stringify({
          ...
          response: {
            ...
            voice: 'alloy'   // ✅ FIXED: use top-level `voice`
          }
        }));
      })
      .catch((e) => {
        ...
        oaSocket.send(JSON.stringify({
          type: 'response.create',
          response: { instructions, modalities: ['audio','text'], voice: 'alloy' }  // ✅ FIXED here too
        }));
      });
  });

  // … rest unchanged …
  // (Twilio <-> OA relay, persist state, etc.)
  // ...
