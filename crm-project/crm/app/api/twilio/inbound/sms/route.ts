    ...
    if (leadId && lead && isRelocationLead(lead)) {
          ...
          if (agentUser?.phone && approvalRow?.id) {
            try {
              ...
              if (accountSid && authToken && fromNumber) {
                ...
                const reviewCode =
                  String(approvalRow.id || '').replace(/-/g, '').slice(0, 12) ||
                  String(approvalToken || '').replace(/[^a-zA-Z0-9]/g, '').slice(0, 12)
      ...
      if (isGuideCheckLead(lead) || isGuideRecoveryConversation) {
        ...
        if (shouldSendGuideLinkBySms) {
          ...
          if (!guideLink) {
            replyText =
              'I am sorry, I am having trouble pulling up the guide link right now. I will have Mike follow up with it directly.'

            return await writeGuideRecoveryReply({
              call_status: 'guide_link_sms_failed_missing_url',
              ...
              sms_resume_step: 'location_timeline',
              sms_detour_reason: 'guide_link_missing_url',
            })
          }
        ...
        const isActiveGuideRecoveryState =
          lead?.sms_state === 'WAITING_FOR_GUIDE_RECEIVED' ||
          lead?.sms_state === 'WAITING_FOR_EMAIL_CONFIRMATION' ||
          lead?.sms_state === 'WAITING_FOR_EMAIL_RESEND_PERMISSION' ||
          lead?.sms_state === 'WAITING_FOR_GUIDE_LINK_REQUEST' ||
          lead?.sms_current_objective === 'confirm_received_guide' ||
          lead?.sms_last_question === 'guide_verification_received' ||
          lead?.sms_last_question === 'guide_resend' ||
          lead?.sms_last_question === 'guide_email_confirm' ||
          lead?.sms_last_question === 'guide_email_permission' ||
          lead?.sms_last_question === 'guide_sms_link'

        if (isGuideCheckLead(lead) && isActiveGuideRecoveryState) {
          if (guideRecoveryYes) {
