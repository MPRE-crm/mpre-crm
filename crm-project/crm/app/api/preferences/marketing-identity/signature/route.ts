...
export async function POST(
  ...
  try {
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            'Signature image must be JPG, PNG or WebP.',
        },
        {
          status: 400,
        }
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            'Signature image must be smaller than 8 MB.',
        },
        {
          status: 400,
        }
    ...
    const storagePath =
      `${profile.org_id}/${profile.id}/signature-${Date.now()}.${extension}`;
    ...
    console.error(
      'marketing signature upload error',
      error
    );
