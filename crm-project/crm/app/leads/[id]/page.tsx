...
export default function EditLeadPage() {
  ...
  useEffect(() => {
    ...
    const fetchLead = async () => {
      const { data, error } = await supabase
        .from('leads')
        .select('first_name, last_name, name, email, phone, status, source, appointment_date, appointment_hour, appointment_minute, appointment_ampm')
        .eq('id', id)
        .single();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      await fetch('/api/zapier', {
        ...
        body: JSON.stringify({ id, first_name, email, phone, status }),
      });
  ...
  const generateICSFile = (
    ...
    const dateString = `${appointmentDate}T${String(hour24).padStart(
      2,
      '0'
