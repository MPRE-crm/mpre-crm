...
export async function sendText({
  ...
  try {
    let lead: any = null;
    let governorDecision: ReturnType<typeof evaluateLeadContactState> | null =
      null;
    ...
    if (leadId) {
      ...
      if (!lead) {
        return {
          success: false,
          error: "Lead not found before SMS.",
        };
      }

      if (!bypassGovernor) {
        governorDecision = evaluateLeadContactState(lead, {
          now: new Date(),
        });

        if (!governorDecision.should_text) {
          console.error(
            "Text blocked by governor for lead",
            leadId,
            governorDecision.reason_codes
          );
    ...
    if (leadId && lead) {
      ...
      const currentCount =
        lead?.texts_today_date === today
          ? Number(lead?.texts_today_count || 0)
          : 0;
      ...
      const { error: updateError } = await supabaseAdmin
        ...
        .update({
          ...
          next_contact_at: bypassGovernor ? null : effectiveDecision.next_contact_at,
      ...
      if (updateError) {
        console.error("Failed to update lead after SMS:", updateError.message);
      }
      ...
      if (messageInsertError) {
        console.error("Failed to log outgoing SMS:", messageInsertError.message);
      }
    }
