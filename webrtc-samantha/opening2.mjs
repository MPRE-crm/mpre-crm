...
const OPENING2_TEXT =
  "Great — you're connected with MPRE Boise, powered by Homes of Idaho. Just so I can help you best, are you calling about buying, selling, or investing in real estate?";

// OUTPUT FILE — EXACT FILENAME YOU WANT
const outPcmPath = path.join(
  __dirname,
  "greetings",
  "mpre-boise-greeting.pcm"
);

// ------------------------
// PCM TRIM HELPER (24kHz SAFE)
// ------------------------
...
async function main() {
  ...
  console.log("🎧 Connecting to OpenAI Realtime TTS for Opening2…");
  ...
  ws.on("open", () => {
    ...
    console.log("➡️ Sending response.create for strict Opening2 text…");
...
`.trim();
  ...
  ws.on("close", () => {
    if (!finished) {
      ...
      console.error(
        `⚠️ WebSocket closed before Opening2 was finished. Bytes captured so far: ${total}`
      );
      process.exit(1);
    } else {
      console.log("🔚 Realtime TTS capture complete for Opening2.");
      process.exit(0);
    }
  });
...
main().catch((err) => {
  console.error("❌ Unhandled error in Opening2 generator:", err);
  process.exit(1);
});
